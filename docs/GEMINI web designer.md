...
Three (or as much as the value propositions are) cards derived from the user's 3 value propositions. These must feel like \*\*functional software micro-UIs\*\*, not static marketing cards. Each card gets one of these interaction patterns:
...
3 or 4 full-screen cards that stack on scroll.  
...
\#\#\# F. SOCIAL — "  
Replaces the pricing grid with a curated, high-fidelity social gallery. This section must feel like an interactive exhibition.

Gallery Logic: A 4-column grid (or masonry on mobile) featuring images sourced from Unsplash using the preset’s imageMood.

Interaction — "Kinetic Parallax": \* Implement a multi-layered parallax effect. As the user scrolls, the even-numbered images move at a y speed of 0.2, while odd-numbered images move at 0.1.

Mouse-Follow: On desktop, implement a subtle tilt effect using GSAP: images should respond to the mouse position with a maximum rotation of 2deg and a slight translation.

Social Connectivity: Below the gallery, a centered row of minimalist social links: Instagram, TikTok, Facebook.

Style: Lucide icons followed by the handle name in the preset's Data font (monospace).

Animation: Magnetic hover effect with a sliding background underline.

\#\#\# G. FAQ —   
A minimalist, high-density information section to resolve friction.  
Structure: A vertical stack of 5 accordion items.Interaction: \* Smooth expansion using gsap.fromTo() on the height property with power2.inOut.The "+" icon must rotate 45^ circ and morph into an "x" upon activation.State: Only one item open at a time (exclusive accordion logic).Content: Generate 5 context-aware questions and answers based on the brand's purpose and value propositions.Closing CTA: A standalone, large-scale button below the FAQ: "Contattami per altre domande →".Style: Heavy weight, accent color, magnetic interaction.

\#\#\# H. FOOTER  
\- Deep dark-colored background, \`rounded-t-\[4rem\]\`.  
\- Grid layout: Brand name \+ tagline, navigation columns, legal links, social links.  
...
\#\# CONTENT LOGIC & SOURCE HIERARCHY (NEVER OVERRIDE)

This protocol defines how text is sourced and generated. The agent must act as a Content Architect, ensuring brand consistency. 

Primary Source (The User Archive): If the user provides an additional file (e.g., copy.txt, brand\_voice.md), this file becomes the Single Source of Truth. Every string, headline, and value proposition must be extracted from here first. 
Here the name of the specific file used as primary source for this project: copy-fz.md

Mapping Protocol: \* Scan the user-provided file for keywords matching the components (Hero, Features, Philosophy, FAQ). 

If a specific section's copy is found, use it verbatim without AI "smoothing" or generic alterations. 

Synthetic Infill (The Inspiration Layer): For any component or field not explicitly defined in the user's file, the agent must generate copy that:

Adopts the Tone: Extrapolates the "voice" found in the user's text. 

Aesthetic Alignment: Uses the vocabulary and "identity" of the selected Aesthetic Preset. 

Example: If the user's text is clinical but missing a "Protocol" description, the agent generates one using the technical rigor of "Organic Tech" or "Brutalist Signal". 

Validation: All generated text must remain "Cinematic" and "Professional", avoiding any generic marketing fluff.
